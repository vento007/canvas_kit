...
/// Grid background that can also draw a thick frame on the edges of a given
/// world `bounds` rectangle, but only when that edge is visible in the current
/// viewport (based on `transform`).
class BoundsGridBackground extends StatelessWidget {
...
class _BoundsGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Compute the bounds rectangle in screen space and align to pixel edges to reduce 1px artifacts
    ...
    // Always draw the thick frame around the transformed bounds rect.
    ...
    // Draw frame as four filled rects INSIDE the bounds to avoid any outside bleed
    if (w > 0) {
      // Left
      canvas.drawRect(
